...
<cac:AllowanceCharge>
    ...
    <cbc:Amount currencyID="{{($invoice->TypeCurrency != NULL)?$invoice->TypeCurrency->code:$invoice->TypeCurrencyDefault->code}}">{{number_format($allowanceCharge->amount, 2, '.', '')}}</cbc:Amount>
    @if ($allowanceCharge->base_amount)
    <cbc:BaseAmount currencyID="{{($invoice->TypeCurrency != NULL)?$invoice->TypeCurrency->code:$invoice->TypeCurrencyDefault->code}}">{{number_format($allowanceCharge->base_amount, 2, '.', '')}}</cbc:BaseAmount>
    @endif
</cac:AllowanceCharge>
@endforeach