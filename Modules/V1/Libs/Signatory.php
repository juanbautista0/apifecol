...
class Signatory
{
     ...
     * Xml signed
     */
    public function Sign(string $certificatePath, string $certificatePassword, string $xmlUnsigned, string $UUID, string $doctype)
    {

        $pfx = file_get_contents($certificatePath);
        openssl_pkcs12_read($pfx, $key, $certificatePassword);
        ...
        return $this->insertaFirma($xmlUnsigned, $doctype);
    }
    ...
    public function insertaFirma($xml, $doctype)
    {
        ...
        return $signed;*/
        $buscar    = '<ext:ExtensionContent>__SIGNATURE__</ext:ExtensionContent>';
